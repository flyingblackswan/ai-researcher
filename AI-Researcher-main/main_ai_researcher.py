...
def main_ai_researcher(input, reference, mode):
    ...
    port = int(os.getenv("PORT") or "7020")
    ...
    match mode:
        case 'Detailed Idea Description':
            ...
            if global_state.INIT_FLAG is False:
                ...
                # Compute predicted artifact paths (mirrors run_infer_plan)
                model_norm = COMPLETION_MODEL.replace("/", "__")
                local_root = os.path.join(os.getcwd(), "workplace_paper", f"task_{instance_id}_{model_norm}", args.workplace_name)
                cache_dir = f"cache_{instance_id}_{model_norm}"
                metachain_log = f"log_{instance_id}"
                ...
                result = {
                    ...
                    "paths": {
                        ...
                        "metachain_log": os.path.join(os.getcwd(), metachain_log),
                    },
                    "metrics": {},
                }
                global_state.INIT_FLAG = False
